...
function promptResultError(result: unknown): string | undefined {
...
// === Managed slash command files ===
//
// The server plugin idempotently installs OpenCode custom command files so
// `/loop`, `/loops`, `/stop-loop`, and `/schedule-standalone-session` work in
// every OpenCode surface that reads `~/.config/opencode/commands/` (terminal
// TUI and Desktop). Files are marker-guarded: we only create or update files
// we own, and we never touch files a user created or edited (marker removed).
//
// Opt out per install with plugin options: ["opencode-routines", {"commands": false}]

const MANAGED_COMMAND_MARKER = "managed-by: opencode-routines"

const MANAGED_COMMANDS: Record<string, string> = {
  "loop.md": `---
  ...
  for (const [filename, content] of Object.entries(MANAGED_COMMANDS)) {
    ...
    try {
      if (existsSync(dest)) {
        ...
        // User-owned file (no marker): never touch it.
        if (!current.includes(MANAGED_COMMAND_MARKER)) continue
        // Ours and current: no-op to avoid spurious mtime bumps/watch reloads.
      ...
      console.error(`[opencode-routines] failed to install command ${filename}:`, error)
    }
  }
}
...
export const SchedulerPlugin: Plugin = async (input, options) => {
  const client = input?.client as unknown as RoutinePromptClient

  if ((options as { commands?: boolean } | undefined)?.commands !== false) {
    syncManagedCommands()
  }
